...
class _About extends StatelessWidget{
  ...
  Widget build(BuildContext c)=>LayoutBuilder(builder:(c,x){
    ...
    final story=Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      const Text('I like turning ideas into clear, useful digital experiences.',style:TextStyle(fontSize:mobile?32:46,height:1.08,fontWeight:FontWeight.w900,letterSpacing:-1.2)),
    ...
    if(mobile)return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[story,const SizedBox(height:30),visual]);
    return Row(crossAxisAlignment:CrossAxisAlignment.center,children:[Expanded(flex:6,child:story),const SizedBox(width:52),Expanded(flex:5,child:visual)]);
  });
}
...
class _CoreNode extends StatelessWidget{const _CoreNode();@override Widget build(BuildContext c)=>Container(width:112,height:112,decoration:BoxDecoration(shape:BoxShape.circle,color:const Color(0xFFF4EFE7),boxShadow:const [BoxShadow(color:Color(0x558E857A),blurRadius:34,spreadRadius:2)]),child:const Column(mainAxisAlignment:MainAxisAlignment.center,children:[Icon(Icons.hub_outlined,size:28),SizedBox(height:6),Text('FAY',style:TextStyle(fontWeight:FontWeight.w900,letterSpacing:1.4))]));}
...
class _OrbitPainter extends CustomPainter{
  @override void paint(Canvas canvas,Size size){
    final paint=Paint()..color=const Color(0xFF5A5651)..strokeWidth=1.2..style=PaintingStyle.stroke;
    final c=Offset(size.width/2,size.height/2);
    canvas.drawCircle(c,size.shortestSide*.28,paint);
    canvas.drawCircle(c,size.shortestSide*.39,paint..color=const Color(0xFF343230));
    final line=Paint()..color=const Color(0xFF4A4742)..strokeWidth=1;
    canvas.drawLine(c,Offset(size.width*.18,size.height*.18),line);
    canvas.drawLine(c,Offset(size.width*.82,size.height*.22),line);
    canvas.drawLine(c,Offset(size.width*.2,size.height*.78),line);
    canvas.drawLine(c,Offset(size.width*.82,size.height*.8),line);
  }
