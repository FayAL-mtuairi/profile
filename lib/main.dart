            ...
            actions:mobile
                ? [
                    PopupMenuButton<String>(
                      ...
                      onSelected:(v){if(v=='about')go(about);if(v=='training')go(training);if(v=='projects')go(projects);if(v=='skills')go(skills);if(v=='contact')go(contact);},
                      itemBuilder:(c)=>const [
                ...
                : [_Nav('About',()=>go(about)),_Nav('Training',()=>go(training)),_Nav('Projects',()=>go(projects)),_Nav('Skills',()=>go(skills)),_Nav('Contact',()=>go(contact)),const SizedBox(width:16)],
          ),
          SliverToBoxAdapter(
            child:Center(
...
class _ScrollRevealState extends State<_ScrollReveal> with SingleTickerProviderStateMixin{
  ...
  bool started=false;
  @override void initState(){super.initState();controller=AnimationController(vsync:this,duration:const Duration(milliseconds:720));fade=CurvedAnimation(parent:controller,curve:Curves.easeOut);slide=Tween<Offset>(begin:const Offset(0,.09),end:Offset.zero).animate(CurvedAnimation(parent:controller,curve:Curves.easeOutCubic));WidgetsBinding.instance.addPostFrameCallback((_)=>_check());}
  void _check(){if(!mounted||started)return;final box=context.findRenderObject();if(box is RenderBox){final top=box.localToGlobal(Offset.zero).dy;final h=MediaQuery.sizeOf(context).height;if(top<h*.88){started=true;controller.forward();}}}
  @override Widget build(BuildContext context){WidgetsBinding.instance.addPostFrameCallback((_)=>_check());return NotificationListener<ScrollNotification>(onNotification:(_){_check();return false;},child:FadeTransition(opacity:fade,child:SlideTransition(position:slide,child:widget.child)));}
...
class _About extends StatelessWidget{
  ...
  Widget build(BuildContext c)=>LayoutBuilder(builder:(c,x){
    ...
    final story=Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      ...
      const Wrap(spacing:10,runSpacing:10,children:[_AboutPill(Icons.insights_outlined,'Data Analytics'),_AboutPill(Icons.phone_iphone_rounded,'Flutter'),_AboutPill(Icons.memory_outlined,'Practical AI'),_AboutPill(Icons.design_services_outlined,'UI thinking')]),
    ]);
...
class _AboutPill extends StatelessWidget{final IconData icon;final String text;const _AboutPill(this.icon,this.text);@override Widget build(BuildContext c)=>Container(padding:const EdgeInsets.symmetric(horizontal:14,vertical:10),decoration:BoxDecoration(border:Border.all(color:const Color(0xFF8E857A)),borderRadius:BorderRadius.circular(22)),child:Row(mainAxisSize:MainAxisSize.min,children:[Icon(icon,size:17),const SizedBox(width:8),Text(text,style:const TextStyle(fontWeight:FontWeight.w700))]));}
class _CoreNode extends StatelessWidget{const _CoreNode();@override Widget build(BuildContext c)=>Container(width:112,height:112,decoration:const BoxDecoration(shape:BoxShape.circle,color:Color(0xFFF4EFE7),boxShadow:[BoxShadow(color:Color(0x558E857A),blurRadius:34,spreadRadius:2)]),child:const Column(mainAxisAlignment:MainAxisAlignment.center,children:[Icon(Icons.hub_outlined,size:28),SizedBox(height:6),Text('FAY',style:TextStyle(fontWeight:FontWeight.w900,letterSpacing:1.4))]));}
class _SkillNode extends StatelessWidget{final String label;final IconData icon;const _SkillNode({required this.label,required this.icon});@override Widget build(BuildContext c)=>Container(padding:const EdgeInsets.symmetric(horizontal:13,vertical:10),decoration:BoxDecoration(color:const Color(0xFF2A2927),borderRadius:BorderRadius.circular(20),border:Border.all(color:const Color(0xFF4A4844))),child:Row(mainAxisSize:MainAxisSize.min,children:[Icon(icon,size:16,color:const Color(0xFFF4EFE7)),const SizedBox(width:7),Text(label,style:const TextStyle(color:Color(0xFFF4EFE7),fontSize:11,fontWeight:FontWeight.w800,letterSpacing:1.1))]));}
class _OrbitPainter extends CustomPainter{@override void paint(Canvas canvas,Size size){final p1=Paint()..color=const Color(0xFF5A5651)..strokeWidth=1.2..style=PaintingStyle.stroke;final c=Offset(size.width/2,size.height/2);canvas.drawCircle(c,size.shortestSide*.28,p1);final p2=Paint()..color=const Color(0xFF343230)..strokeWidth=1.2..style=PaintingStyle.stroke;canvas.drawCircle(c,size.shortestSide*.39,p2);final line=Paint()..color=const Color(0xFF4A4742)..strokeWidth=1;canvas.drawLine(c,Offset(size.width*.18,size.height*.18),line);canvas.drawLine(c,Offset(size.width*.82,size.height*.22),line);canvas.drawLine(c,Offset(size.width*.2,size.height*.78),line);canvas.drawLine(c,Offset(size.width*.82,size.height*.8),line);}@override bool shouldRepaint(covariant CustomPainter oldDelegate)=>false;}
