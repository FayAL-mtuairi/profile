...
class _ProjectCard extends StatelessWidget{final _Project p;const _ProjectCard(this.p);@override Widget build(BuildContext c){final images={'Weather Dashboard':'assets/projects/weather.jpg','Sales Dashboard':'assets/projects/sales.jpg'};final image=images[p.title];return Container(decoration:BoxDecoration(color:const Color(0xFFE9E1D6),border:Border.all(color:const Color(0xFFBDB4A8))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[if(image!=null)AspectRatio(aspectRatio:620/345,child:Image.asset(image,fit:BoxFit.cover)),Padding(padding:const EdgeInsets.all(24),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(p.category.toUpperCase(),style:const TextStyle(fontSize:11,fontWeight:FontWeight.w800,letterSpacing:1.5)),const SizedBox(height:18),Text(p.title,style:const TextStyle(fontSize:25,fontWeight:FontWeight.w900)),const SizedBox(height:5),Text(p.type,style:const TextStyle(color:Color(0xFF6C655D),fontWeight:FontWeight.w600)),const SizedBox(height:18),Text(p.description,style:const TextStyle(fontSize:15,height:1.65,color:Color(0xFF4E4943))),const SizedBox(height:20),Wrap(spacing:7,runSpacing:7,children:p.tags.map((e)=>_Tag(e)).toList())]))]));}}
