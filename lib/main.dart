  ...
  @override Widget build(BuildContext context){
    ...
    return SizedBox(
      ...
      child:Stack(clipBehavior:Clip.none,children:[
        ...
        Positioned(left:mobile?6:-10,top:mobile?50:68,child:const _TechCard(icon:Icons.insights_outlined,label:'Power BI')),
        Positioned(right:mobile?4:-14,top:mobile?18:18,child:const _TechCard(icon:Icons.code_rounded,label:'Python')),
        Positioned(left:mobile?8:-12,top:mobile?195:205,child:const _TechCard(icon:Icons.storage_rounded,label:'SQL')),
        Positioned(right:mobile?4:-20,top:mobile?182:190,child:const _TechCard(icon:Icons.phone_iphone_rounded,label:'Flutter')),
        Positioned(right:mobile?18:-2,bottom:mobile?40:48,child:const _TechCard(icon:Icons.table_chart_outlined,label:'Excel')),
        Positioned(left:mobile?24:10,bottom:mobile?14:4,child:Container(width:mobile?160:190,padding:const EdgeInsets.all(16),decoration:BoxDecoration(color:_cream,border:Border.all(color:_line),borderRadius:BorderRadius.circular(18)),child:const Row(children:[Icon(Icons.show_chart_rounded,color:_olive,size:26),SizedBox(width:12),Expanded(child:Text('Turn data into insights',style:TextStyle(fontSize:13,height:1.3,fontWeight:FontWeight.w600,color:_ink)))]))),
      ]),
    );
  }
}
...
class _TechCard extends StatefulWidget{
  final IconData icon;
  final String label;
  const _TechCard({required this.icon,required this.label});
...
class _TechCardState extends State<_TechCard>{
  ...
  @override Widget build(BuildContext context)=>MouseRegion(
    ...
    child:AnimatedContainer(
      ...
      decoration:BoxDecoration(color:_cream,border:Border.all(color:_line),borderRadius:BorderRadius.circular(18),boxShadow:hover?const [BoxShadow(color:Color(0x16000000),blurRadius:18,offset:Offset(0,8))]:const []),
      child:Row(mainAxisSize:MainAxisSize.min,children:[Icon(widget.icon,size:20,color:_olive),const SizedBox(width:9),Text(widget.label,style:const TextStyle(fontSize:13,fontWeight:FontWeight.w600,color:_ink))]),
    ),
  );
}
...
class _About extends StatelessWidget{
  ...
  @override Widget build(BuildContext context)=>LayoutBuilder(builder:(context,constraints){
    ...
    final story=Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      Text('I like turning ideas into clear, useful digital experiences.',style:TextStyle(fontSize:mobile?32:46,height:1.08,fontWeight:FontWeight.w800,letterSpacing:-1.2,color:_ink)),
...
class _ProjectCardState extends State<_ProjectCard>{bool hovered=false;@override Widget build(BuildContext context){final project=widget.project;final cover=project.images.isNotEmpty?project.images.first:null;return MouseRegion(onEnter:(_)=>setState(()=>hovered=true),onExit:(_)=>setState(()=>hovered=false),child:AnimatedContainer(duration:const Duration(milliseconds:180),curve:Curves.easeOut,transform:Matrix4.translationValues(0,hovered?-5:0,0),decoration:BoxDecoration(boxShadow:hovered?const [BoxShadow(color:Color(0x15000000),blurRadius:22,offset:Offset(0,12))]:const []),child:Material(color:_cream,child:InkWell(onTap:()=>Navigator.of(context).push(MaterialPageRoute(builder:(_)=>_ProjectDetails(project))),child:Container(decoration:BoxDecoration(border:Border.all(color:hovered?_olive:_line)),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[if(cover!=null)ClipRect(child:AnimatedScale(scale:hovered?1.025:1,duration:const Duration(milliseconds:220),curve:Curves.easeOut,child:Container(height:210,width:double.infinity,color:const Color(0xFFFBF8F2),padding:const EdgeInsets.all(16),child:Image.asset(cover,fit:BoxFit.contain)))),Padding(padding:const EdgeInsets.all(22),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(project.category.toUpperCase(),style:const TextStyle(fontSize:11,fontWeight:FontWeight.w800,letterSpacing:1.5,color:_olive)),const SizedBox(height:12),Text(project.title,style:const TextStyle(fontSize:26,fontWeight:FontWeight.w800,color:_ink)),const SizedBox(height:7),Text(project.type,style:const TextStyle(color:_muted,fontWeight:FontWeight.w600)),const SizedBox(height:14),Text(project.description,maxLines:3,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:14.5,height:1.55,color:_muted)),const SizedBox(height:16),Wrap(spacing:7,runSpacing:7,children:project.tags.take(3).map((e)=>_Tag(e)).toList()),const SizedBox(height:18),Row(children:[const Text('View project',style:TextStyle(fontWeight:FontWeight.w700,color:_ink)),const SizedBox(width:7),AnimatedSlide(duration:const Duration(milliseconds:180),offset:hovered?const Offset(.18,0):Offset.zero,child:const Icon(Icons.arrow_forward_rounded,size:17,color:_olive))])]))])))))));}}
